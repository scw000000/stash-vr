...
* In-browser VR scene editor
  * Runtime subtitle generation for one selected scene at a time
  * Per-scene generation progress, related SRT deletion, and immediate playback selection
* HereSphere
...
* `CAPTION_PYTHON`
  * Default: `python`
  * Python executable used for runtime subtitle jobs.
* `CAPTION_GENERATOR_PATH`
  * Default: `../python_tools/CaptionGenerator`
  * Path containing `video_caption_generator.py` and its installed dependencies.
* `CAPTION_ENV_FILE`
  * Default: `.env`
  * Dotenv file containing any API keys required by the selected CaptionGenerator services.
</details>
...
### Runtime subtitle generation

Open a scene in the in-browser VR player, then open **Details → Edit → Runtime subtitle generation**. The controls expose CaptionGenerator's language, mode, transcription service/model, and translation service/model choices. A job is scoped to the selected scene's primary video file, and its progress remains visible in that scene's Edit panel.

Generation is launched as a native Stash plugin task, so the same job and progress also appear under **Settings → Tasks**. At startup, Stash-VR installs or updates its small task runner in Stash's configured plugins directory and asks Stash to reload plugins when necessary. Stash owns the resulting process, including task cancellation.

Generated and existing sidecar files matching `<video>.srt` or `<video>.*.srt` are listed in the same section. They can be deleted there and become available in the playback panel's subtitle picker without a Stash rescan.

The Stash process must be able to use `CAPTION_PYTHON`, read `CAPTION_GENERATOR_PATH` and `CAPTION_ENV_FILE`, and write beside the video path reported by Stash. Install the dependencies from CaptionGenerator's `requirements_caption.txt`; local model options need their corresponding optional packages. If Stash or Stash-VR runs in a container, both processes need consistent plugin, CaptionGenerator, dotenv, and media path mounts.
